...
- **Provider-only resources must not mimic adapter verbs.** If a resource
  ...
  (e.g. `list-types`, `list-tables` for commands that genuinely enumerate a
  collection) — never bare `get`, `list`, `create`, `update`, `delete`. This
  avoids user confusion: adapter verbs (`resources get`) and provider verbs
  should not overlap for resources that behave differently across the two
  paths; a compound such as `list-types` does not collide with any adapter verb.
