...
These rules were first codified by the OnCall alert-group rich-shape ADR ([`../adrs/oncall-alert-group-rich-shape/001-rich-shape-and-list-defaults.md`](../adrs/oncall-alert-group-rich-shape/001-rich-shape-and-list-defaults.md)). Future ADRs that introduce new cross-cutting rules SHOULD append them here rather than re-litigating them per provider.
